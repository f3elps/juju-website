...
(() => {
  ...
  const ACCEPT_EXT = /\.(png|jpe?g|webp|avif|heic|heif)$/i;
  function isAccepted(file) {
    if (!file) return false;
    if (file.type && ACCEPT.indexOf(file.type) >= 0) return true;
    // HEIC/HEIF often arrive with an empty or odd MIME type — fall back to
    // the file extension so Apple photos still go through.
  ...
  function save() {
    ...
    Promise.resolve(w(STATE_FILE, JSON.stringify(slots)))
      .catch((e) => { console.error('<image-slot> sidecar write failed (photos may not persist):', e); })
  ...
  // ── Image downscale ─────────────────────────────────────────────────────
  // Some HEIC/HEIF files can't be decoded by createImageBitmap (only Safari
  // does it, and inconsistently). When we detect one, convert it to a JPEG/PNG
  // blob first via the heic2any library (loaded on the page), then proceed.
  function isHeicFile(file) {
    return /image\/hei[cf]/i.test(file.type || '') || /\.(heic|heif)$/i.test(file.name || '');
  }
  // The library tag loads async-ish; if a drop lands before it's parsed, wait
  // a little rather than failing outright.
  async function getHeic2any(timeoutMs) {
  ...
  async function decodeBitmap(file) {
    if (isHeicFile(file)) {
      const conv = await getHeic2any();
      if (!conv) { const e = new Error('HEIC_LIB_MISSING'); e.heic = true; throw e; }
      let converted = null;
      for (const toType of ['image/jpeg', 'image/png']) {
        try {
          const out = await conv({ blob: file, toType: toType, quality: 0.9 });
          converted = Array.isArray(out) ? out[0] : out;
          if (converted) break;
        } catch (err) {
          console.warn('<image-slot> HEIC->' + toType + ' failed:', err);
        }
      }
      if (!converted) { const e = new Error('HEIC_CONVERT_FAILED'); e.heic = true; throw e; }
      file = converted;
    }
  ...
  const stylesheet =
    ...
    '  text-align:center;padding:12px;box-sizing:border-box;z-index:4;' +
    '  color:#b03860;font:600 13px/1.4 system-ui,-apple-system,sans-serif;' +
    '  background:rgba(255,247,250,.92);backdrop-filter:blur(3px)}';
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '</div>' +
        '<div class="ctl"><button data-act="reframe" title="Ajustar enquadramento">✥ Ajustar</button>' +
        '  <button data-act="replace" title="Trocar imagem">Trocar</button>' +
    ...
    async _ingest(file) {
      ...
      if (heic) this._setBusy('Convertendo HEIC… 🪄');
      try {
        const w = this.clientWidth || this.offsetWidth || MAX_DIM;
        const url = await toDataUrl(file, w);
        if (gen !== this._gen) { this._setBusy(null); return; }
        ...
        this._setBusy(null);
        this._setError(err && err.heic
          ? 'Não consegui converter esse HEIC. Tente reenviar, ou exporte a foto como JPG no celular.'
    ...
    _setBusy(msg) {
      if (this._busyEl) { this._busyEl.remove(); this._busyEl = null; }
      if (!msg) return;
      const d = document.createElement('div');
      d.className = 'busy'; d.textContent = msg;
